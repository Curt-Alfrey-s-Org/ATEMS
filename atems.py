...
def create_app():
    ...
    # When deployed under subpath (e.g. /app/atems), set SCRIPT_NAME so url_for generates correct links
    app_root = (os.getenv("APPLICATION_ROOT", "/app/atems") or "").strip().rstrip("/")
    ...
    with app.app_context():
        ...
        # Ensure guest user exists for demo
        if User.query.filter_by(username="guest").first() is None:
            ...
            db.session.commit()
            logger.info("Created guest demo user (guest / guest123).")
